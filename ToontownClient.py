...
class CommonContext:
    ...
    async def send_location_scouts(self, locations: typing.List) -> None:
        """ send `LoactionScouts` packet to get item information """
        payload = {
            'cmd': 'LocationScouts',
            'locations': locations
        }
...
def run_as_toontownclient():
    class ToontownContext(CommonContext):
        # Text Mode to use !hint and such with games that have no text entry
        #tags = CommonContext.tags
        game = "Toontown"  # empty matches any game since 0.3.2
        items_handling = 0b111  # receive all items for /received
        want_slot_data = True  # use game specific slot_data

        async def server_auth(self, password_requested: bool = False):
            if password_requested and not self.password:
                await super(ToontownContext, self).server_auth(password_requested)
    ...
    async def main(args):
        ctx = ToontownContext(args.connect, args.password)
        if args.path:
            async_start(ctx.connect(args.path), name="connecting")
        ctx.auth = args.name
        ctx.server_task = asyncio.create_task(server_loop(ctx), name="server loop")
    ...
    parser = get_base_parser(description="Toontown Archipelago Client.")
