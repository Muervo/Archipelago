...
from .Items import ToontownItem, item_data_table, item_table
from .Locations import ToontownLocation, location_data_table, location_table, locked_locations
from .Options import toontown_options
from .Regions import region_data_table
#from .Rules import get_button_rule


class ToontownWebWorld(WebWorld):
    theme = "partyTime"
    
    setup_en = Tutorial(
        tutorial_name="Start Guide",
        description="A guide to playing Toontown.",
        language="English",
...
class ToontownWorld(World):
    """ A wacky toontastic game for everyone! """

    game = "Toontown"
    data_version = 1
    web = ToontownWebWorld()
    option_definitions = toontown_options
    ...
    def create_item(self, name: str) -> ToontownItem:
        return ToontownItem(name, item_data_table[name].type, item_data_table[name].code, self.player)

    def create_items(self) -> None:
        item_pool: List[ToontownItem] = []
    ...
    def set_rules(self) -> None:
        ...
        self.multiworld.completion_condition[self.player] = lambda state: state.has("A Message", self.player)
